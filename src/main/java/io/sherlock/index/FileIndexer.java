    ...
    private static void indexFiles(String searchPath, String indexPath) throws IOException {
        ...
        while (folders.size() > 0) {
            ...
            if (file.isDirectory()) {
                ...
                try (InputStream stream = Files.newInputStream(file.getAbsolutePath()))
               {
...
public class FileIndexer
{
    public static void main(String[] args) {
        //Input folder
        String docsPath = "C://Projects";

        //Output folder
        String indexPath = "C://vertx/indexes";
    ...
    static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException
    {
        try (InputStream stream = Files.newInputStream(file))
        {
            ...
            doc.add(new StringField("path", Paths.get("C://Projects").relativize(file).toString(), Field.Store.YES));
            doc.add(new LongPoint("modified", lastModified));
            doc.add(new TextField("contents", new String(Files.readAllBytes(file)), Store.YES));
